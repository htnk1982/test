"""Note-Sub frontend for v3.3 with automatic physical preparation.

Decisions are observed from a constant-gain -14 LUFS shadow of HarmonicElasticity.
The physical path is independently prepared to -14 LUFS / -2.5 dBTP by the fixed
AUTO order: gain-only -> OPPO -> limiter fallback only after NotFeasible.
"""
...
import offline_peak_stream_v32 as stream
import auto_peak_v33 as auto

VERSION='note-sub-auto-conditioned-3.3.0'
REFERENCE_LUFS=-14.0
INPUT_TP=-2.5
...
def run(source,work,progress=None,*,interrupt_after=None):
    ...
    with legacy._RUNTIME_LOCK,io.job_lock(job/'job.lock'):
        ...
        # Analysis is never peak-limited: it is a non-published observation shadow.
        ...
        # Both analysis and physical paths are the same integrated loudness. Keep
        # the decisions/amounts anchored to the uncompressed shadow; do not let a
        # limiter fallback teach Note-Sub a different event map.
        ...
        report=dict(version=VERSION,prep_route=prep['auto_route'],prep_oppo_used=prep['auto_route']=='OPPO',
            ...
            physical_target_lufs=REFERENCE_LUFS,analysis_anchor_lufs=REFERENCE_LUFS,
            analysis_to_render_gain=amplitude_scale,baseline_metrics=baseline,prepared_metrics=pq,
            ...
            selected_seconds=sum(e['end']-e['start'] for e in events) if chosen else 0.,choices=choices,reasons=reasons,
            analysis_cache=cache,delta_cache=dcache,source_unchanged=True,
            output_metrics=stream.measure(candidate,progress,'NOTE_FINAL'))
