"""Automatic peak policy for OPPO v3.3.

Order is fixed: constant gain (inside OPPO fit) -> OPPO v3.2 -> conventional
limiter only when OPPO explicitly raises NotFeasible. Operational/integrity
errors never trigger a limiter fallback. A separate AUTO receipt makes interrupted
limiter-fallback runs safely resumable without misclassifying the existing output.
"""
from __future__ import annotations
from pathlib import Path
from dataclasses import asdict
import math
import offline_peak_stream_v32 as oppo
import offline_peak_lab as kernel
import distribution_peak as limiter
import note_sub_lab as io

VERSION='auto-peak-v3.3.1'
...
def fit(source,dest,work,target,ceiling,*,progress=None,chunk_seconds=2.0,interrupt_after=None,**kwargs):
    ...
    try:
        ...
        # ONLY an explicit feasibility/naturalness refusal may reach here.
        # RuntimeError/OSError/InterruptedError/integrity errors propagate.
