...
MODULES = ('processed_finish', 'distribution_finish', 'distribution_peak',
           ...
           'natural_finish', 'natural_legacy_finish', 'offline_peak_lab',
           'offline_peak_stream', 'note_sub_conditioned', 'natural_gui')
...
def _backend(preparation):
    if preparation == 'legacy_peak':
        import natural_legacy_finish
        return natural_legacy_finish
    import natural_finish
    return natural_finish


def _work_root():
    return Path(os.environ.get('LOCALAPPDATA', str(Path.home()/'.local/share'))) / 'PDRM_Local_Render_Engine_v1' / 'oppo_finish_v3'
    ...
    with logpath.open('w', encoding='utf-8') as log:
        ...
        try:
            ...
            if args[:1] == ['--bundle-check']:
                ...
                return 0
            if args[:1] == ['--gui-check']:
                from natural_gui import self_check
            ...
            if headless:
                ...
                parser.add_argument('--preparation', choices=('gain_only','legacy_peak'), default='gain_only')
                ...
                backend = _backend(parsed.preparation)
                runargs = list(args)
                if '--preparation' in runargs:
                    pos=runargs.index('--preparation');del runargs[pos:pos+2]
                result = app.main(runargs, backend=backend, default_root=_work_root())
                return result
            from natural_gui import choose_targets
            selection = choose_targets()
            if selection is None:
                print('中止しました。原音は変更していません。')
                return 0
            targets, replace_managed, preparation = selection
            ...
            result = app.main(params, backend=_backend(preparation), default_root=_work_root())
