...
class TargetDialog:
    def __init__(self, root, initial=None, warning='', *, settings_file=None):
        self.root = root
        self.settings_file = settings_file or settings_path().parent.parent/'gui_oppo_v3'/'settings.json'
        self.result = None
        self.variables = {}
        self.root.title('PDRM OPPO — WAV / MP3 出力設定')
        self.root.resizable(False, False)
        self.root.protocol('WM_DELETE_WINDOW', self.cancel)
        outer = ttk.Frame(root, padding=20)
        outer.grid(sticky='nsew')
        ttk.Label(outer, text='オフライン波形最適化・出力条件', font=('Yu Gothic UI', 15, 'bold')).grid(
            row=0, column=0, columnspan=3, sticky='w', pady=(0,12))
        ttk.Label(outer, text='選好された波形最適化を使用します。出力条件は個別指定です。').grid(
        ...
        ttk.Label(outer, text='TPは上限です。条件が厳しすぎる場合は、通常リミッターに切り替えず停止します。',
        ...
        # Explicit, never-automatic frontend choice. Default is reset to gain-only
        # each launch; compatibility mode must be knowingly selected this run.
        for child in list(outer.winfo_children()):
            data = child.grid_info()
            if int(data.get('row',0)) >= 5:
                child.grid_configure(row=int(data['row'])+2)
        self.preparation = tk.StringVar(root, value='一定ゲインのみ（前段リミッターなし）')
        ttk.Label(outer, text='前段の入力準備').grid(row=5,column=0,sticky='w',pady=(10,0))
        ttk.Combobox(outer, textvariable=self.preparation, state='readonly', width=48,
            values=('一定ゲインのみ（前段リミッターなし）',
                    '比較時と同じ前段ピーク整形（明示的な互換モード）')).grid(
                    row=6,column=0,columnspan=3,sticky='ew',pady=(4,8))
    ...
    def submit(self):
        ...
        mode='gain_only' if self.preparation.get().startswith('一定') else 'legacy_peak'
        self.result=(targets,bool(self.replace.get()),mode)
        self.root.destroy()
...
def choose_targets():
    config=settings_path().parent.parent/'gui_oppo_v3'/'settings.json'
    targets,warning=load_settings(config)
    root=tk.Tk()
    dialog=TargetDialog(root,targets,warning,settings_file=config)
    root.mainloop()
...
def self_check(destination):
    ...
    assert dialog.preparation.get().startswith('一定')
    ...
    dest.write_text(json.dumps(dict(status='PASS',targets=expected,
        checks=['4 editable fields','invalid entry blocked','submit and remember','reset','cancel preserves settings','explicit frontend default gain-only']),indent=2),encoding='utf-8')
