    ...
    def __init__(self,root,initial=None,warning='',*,settings_file=None):
        ...
        for row,(label,prefix) in enumerate((('WAV / 24-bit','wav'),('MP3 / 320kbps','mp3')),3):
            ...
            for col,suffix in ((1,'lufs'),(2,'tp')):
                key=prefix+'_'+suffix;var=tk.StringVar(root,value=f'{getattr(initial,key):g}');self.variables[key]=var;lo,hi=(-30,-8) if suffix=='lufs' else (-12,-1)
                ctl=ttk.Spinbox(outer,textvariable=var,from_=lo,to=hi,increment=.1,format='%.1f',width=16);ctl.grid(row=row,column=col,sticky='w',padx=(0,14),pady=8,ipady=3)
...
def self_check(destination):
    dest=Path(destination);settings=dest.with_suffix('.settings.json');root=tk.Tk();d=TargetDialog(root,settings_file=settings);root.update();expected=dict(wav_lufs=-15.5,wav_tp=-2.5,mp3_lufs=-16.5,mp3_tp=-3.5)
    ...
    dest.write_text(json.dumps(dict(status='PASS',targets=expected,automatic_policy='GAIN_ONLY_OPPO_LIMITER',execution='SPARSE_EXACT_CONTEXT',checks=['4 target fields','no manual peak-mode selection','invalid blocked','settings remembered']),indent=2),encoding='utf-8');settings.unlink(missing_ok=True);return 0
