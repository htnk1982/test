...
CONFIG = dict(analysis_sr=4000, frame_seconds=0.192, hop_seconds=0.020,
              ...
              min_pitch_hz=30.0, max_pitch_hz=180.0,
              min_sub_hz=30.0, max_sub_hz=65.0,
              periodicity_min=0.84, low_periodicity_min=0.88,
              agreement_cents=35.0, harmonic_fraction_min=0.52,
              harmonic_support_ratio=0.16, min_harmonics=2,
              max_quarter_range_db=8.0, min_event_seconds=0.12,
              max_step_cents=75.0, max_event_span_cents=110.0,
              fade_seconds=0.060, desired_partial_ratio=0.50,
              max_added_peak=0.080, max_relative_peak=0.70,
              already_full_ratio=0.90, target_lufs=-14.0,
              pcm_tp_ceiling=-1.5, codec_tp_ceiling=-1.0,
              max_matching_gain_change_db=0.50,
              leakage_above_110_limit_db=-35.0,
              layer_below_20_limit_db=-20.0,
              scales=[1.0, 0.5, 0.25])
...
def atomic_wav(path, data, sr, subtype='FLOAT'):
    path = Path(path)
    fd, tmp = tempfile.mkstemp(prefix='.pdrm_', suffix='.wav', dir=path.parent)
    os.close(fd)
    try:
...
def job_lock(path):
    ...
    try:
        ...
        f.close()
        raise RuntimeError('This job is already running. Do not start another copy.') from exc
    try: yield
    finally: f.close()
...
class Progress:
    ...
    def set(self, stage, done=0, total=0):
        ...
        self.state = dict(stage=stage, done=done, total=total)
...
def nsdf_pitch(x, sr=4000):
    ...
    # Exact overlap energy denominator, no window-induced autocorrelation bias.
    ac = signal.fftconvolve(x, x[::-1], mode='full')[n-1:]
    cs = np.r_[0.0, np.cumsum(x*x)]
    lo = max(2, int(sr/CONFIG['max_pitch_hz']))
    hi = min(n//2, int(sr/CONFIG['min_pitch_hz'])+1)
    lag = np.arange(hi+2)
    den = cs[n-lag] + cs[n] - cs[lag]
    curve = 2*ac[:hi+2]/np.maximum(den, EPS)
    peaks, _ = signal.find_peaks(curve[lo:hi+1])
    peaks = peaks + lo
    if not len(peaks): return None, 0.0
    best = float(np.max(curve[peaks]))
    good = peaks[curve[peaks] >= max(0.70, best*0.93)]
    ...
    hz = sr/(p+shift)
    if not CONFIG['min_pitch_hz'] <= hz <= CONFIG['max_pitch_hz']:
        return None, float(b)
...
def analyze_frame(wide, low, stereo, sub, t):
    ...
    f, p = nsdf_pitch(wide)
    f2, p2 = nsdf_pitch(low)
    out.update(periodicity=p, low_periodicity=p2)
    ...
    largest = max(amps)
    support = sum(a>=largest*CONFIG['harmonic_support_ratio'] for a in amps)
    ...
    # A pure sine kick and a pure sine bass cannot be distinguished by pitch alone.
    # In this first prototype, do not invent an instrument classification for them.
    ...
    if target is None:
        out['reason'] = 'outside_single_octave_sub_range'; return out
    existing, _ = component(np.mean(stereo, axis=1), target, CONFIG['analysis_sr'])
    desired = min(largest*CONFIG['desired_partial_ratio'], CONFIG['max_added_peak'],
                  rms*CONFIG['max_relative_peak'])
    sub_rms = float(np.sqrt(np.mean(sub*sub)))
    out.update(sub_hz=float(target), existing_target_amplitude=existing,
               existing_sub_rms=sub_rms, desired_amplitude=desired)
    ...
    if amplitude < 1e-5:
        out.update(state='KEEP', reason='negligible_addition'); return out
    out.update(state='REINFORCE' if existing>desired*0.20 else 'SYNTHESIZE',
               reason='eligible_tonal_interval', amplitude=float(amplitude))
    return out
...
def read_analysis(path, a_seconds, b_seconds):
    ...
    a = max(0, int(math.floor(a_seconds*sr/down))*down)
    b = min(info.frames, int(math.ceil(b_seconds*sr)))
...
def collect_frames(path, job, progress, interrupt_after=None):
    ...
    for i in range(count):
        ...
        cache = job/'analysis'/f'frames_{i:05d}.json'
        record = None
        if cache.exists():
            try:
                v = read_json(cache)
        ...
        if record is not None:
            reused += 1; rows.extend(record); continue
...
def event_wave(event, times, phase=None):
    """Analytical integral of a piecewise-linear frequency trajectory; no chunk resets."""
    ...
    dt = times-knots[j]; width = knots[j+1]-knots[j]
...
def make_events(rows, path):
    ...
    for g in groups:
        ...
        e = dict(start=start, end=end, times=tt.tolist(), frequencies=freq.tolist(),
                 ...
                 median_f0_hz=float(np.median([r['f0_hz'] for r in g])),
                 median_sub_hz=float(np.median(freq)), phase=0.0)
        # Phase is selected once per event. No arbitrary phase changes at render boundaries.
        data, origin = read_analysis(path, start, min(end, start+2.0))
        t = origin+np.arange(len(data))/CONFIG['analysis_sr']
        keep = (t>=start)&(t<end); data=data[keep]; t=t[keep]
        if not len(t): continue
        best = None
        for phase in np.arange(8)*np.pi/4:
            y = event_wave(e, t, float(phase))
            corr = float(np.sum(y*np.mean(data, axis=1)))
            peak = float(np.max(np.abs(data+y[:,None])))
            cost = (corr < -1e-8, peak, -corr, float(phase))
...
def layer_chunk(events, start_frame, frames, sr, scale=1.0):
    ...
    if frames:
        for e in events:
            ...
            out[mask] += event_wave(e, t[mask])*scale
    return out


def k_filter(sr):
    # Isolated adapter: compare against installed pyloudnorm in selfcheck/tests.
    meter = pyln.Meter(sr)
    stages = list(meter._filters.values())
    return [(np.asarray(s.b)*s.passband_gain, np.asarray(s.a)) for s in stages]
...
def measure(path, progress=None, label='MEASURE'):
    ...
    with sf.SoundFile(path) as fin:
        for a in range(0, info.frames, width):
            ...
            fin.seek(left); xp=fin.read(right-left,dtype='float64',always_2d=True); finite(xp)
            x=xp[a-left:b-left]
            ...
            sumsq+=float(np.sum(x*x)); seen+=x.size
            weighted=x
            for k,(bb,aa) in enumerate(coeff):
                weighted,zi[k]=signal.lfilter(bb,aa,weighted,axis=0,zi=zi[k])
            ...
            # Fixed FFT resolution and zero padding only for the final very short block.
            nfft=16384; xx=x
            ...
            part=np.mean(ps,axis=1)*(b-a)
            psum=part if psum is None else psum+part; fgrid=ff
    expected=max(0,int(np.round((info.duration-.4)/.1))+1)
    if len(blocks)<expected and len(tail): blocks.append(float(np.sum(tail)/win))
    z=np.asarray(blocks); lev=-.691+10*np.log10(np.maximum(z,EPS))
    good=z[lev>=-70]
    lufs=None
    if len(good):
        threshold=-.691+10*np.log10(np.mean(good))-10
        good=z[(lev>-70)&(lev>threshold)]
        if len(good): lufs=float(-.691+10*np.log10(np.mean(good)))
    total=float(np.sum(psum)) if psum is not None else 0.0
    def ratio(mask):
        return None if total<EPS else float(10*np.log10(max(float(np.sum(psum[mask]))/total,EPS)))
...
def valid_audio_cache(path, marker):
    try:
        v=read_json(marker)
        return path.exists() and file_hash(path)==v['sha256']
...
def render(path, job, events, progress, scale, gain=1.0, label='candidate', interrupt_after=None):
    ...
    with sf.SoundFile(path) as f:
        for i,a in enumerate(range(0,info.frames,width)):
            progress.set('RENDER_'+label,i,count)
            b=min(info.frames,a+width); p=chunks/f'{i:05d}.wav'; marker=p.with_suffix('.json')
            if valid_audio_cache(p,marker): reused+=1
                ...
                if interrupt_after is not None and computed>=interrupt_after:
                    raise RuntimeError('TEST_INTERRUPTION_AFTER_RENDER_COMMIT')
            committed.append(p)
    ...
    if not valid_audio_cache(out,marker):
        ...
        os.replace(tmp,out); atomic_json(marker,dict(sha256=file_hash(out)))
...
def validate_manifest(path):
    path=Path(path).resolve(); path=path/'manifest.json' if path.is_dir() else path
    m=read_json(path)
    if m.get('experiment_id')!='PDRM-v0.6-Round9-HarmonicLoudness-exp1':
        raise ValueError('Select the accepted Round 9 manifest.json')
    job=path.parent
    if read_json(job/'LAB_INTERNAL/blind_mapping.json').get('C')!='HarmonicElasticity':
        raise ValueError('This job does not map C to HarmonicElasticity')
...
def run_job(source, root, write_mp3=True, expected_hash=None, interrupt_after=None):
    ...
    if root==source.parent or source.parent in root.parents:
        raise ValueError('Output must be outside the original audio/job directory')
    info=sf.info(source)
    ...
    with job_lock(job/'job.lock'), Progress(job) as progress:
        try:
            ...
            if abs(base['lufs_i']-CONFIG['target_lufs'])>0.10:
                raise ValueError('Use the already level-matched C near -14 LUFS, not another master')
            if base['true_peak_dbtp_estimate']>CONFIG['pcm_tp_ceiling']:
                raise ValueError('Baseline already exceeds the experimental peak ceiling; no limiter is added')
            ...
            delta,dcache=render(source,job,events,progress,1.0,label='delta')
            dm=measure(delta,progress,'MEASURE_ADDED_LAYER')
            # No-ops are an explicit result, never a disguised success in sound quality.
            if events:
                ...
                for i,scale in enumerate(CONFIG['scales']):
                    ...
                    met=measure(raw,progress,'MEASURE_RAW')
                    gain_db=CONFIG['target_lufs']-met['lufs_i']; gain=10**(gain_db/20)
            ...
            # A failed trial does not publish its rejected sub layer as the selected delta.
            selected_delta,_=render(source,job,events if chosen else [],progress,selected_scale,label='selected_delta')
            # selected_delta label renders dry+delta; write the actual selected mono delta explicitly below.
            stage=job/'publish_staging'
            if stage.exists(): shutil.rmtree(stage)
            stage.mkdir(); shutil.copyfile(source,stage/'CONTROL_C.wav')
            shutil.copyfile(chosen or source,stage/'SUB_AUGMENTED.wav')
            ...
            if chosen: final_met=measure(chosen,progress,'FINAL_METRICS')
            else: final_met=base
            report=dict(version=VERSION,status=status,identity=ident,source_name=source.name,
                        ...
                        selected_scale=selected_scale,matched_output_gain_db=(CONFIG['target_lufs']-choices[-1]['raw_metrics']['lufs_i']) if chosen else 0.0,
                        ...
                        frame_reason_counts=dict(Counter(r['reason'] for r in rows)),
            ...
            atomic_json(stage/'SUB_REPORT.json',report)
            ...
            progress.set('COMPLETE',1,1); print(status, '\n', final/'SUB_REPORT.json',flush=True)
            return report,final
            ...
            print('FAILED. Do not repeat blindly. Diagnostic:',job/'SUB_FAILURE.json',flush=True)
            raise
...
def main():
    ...
    if root.resolve()==source_job or source_job in root.resolve().parents:
        raise ValueError('Do not put output inside the original Round9 job')
