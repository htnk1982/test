...
After peak feasibility, a bounded common-channel gain field can return removed
short-time energy into safe neighbouring samples of the ORIGINAL waveform. This
is energy redistribution, not makeup to the peak: samples without headroom get
zero added gain and the TP box remains hard.
...
VERSION='offline-peak-context-0.1.1'
...
class ContextConfig:
    ...
    envelope_restore_max_gain: float=0.12
    envelope_restore_iterations: int=6
    iterations: int=320
    def validate(self,sr:int):
        if sr<=0:raise ValueError('Invalid sample rate')
        if not self.contexts_ms or any((not math.isfinite(float(v)) or v<64 or v>1500) for v in self.contexts_ms):raise ValueError('Invalid context sequence')
        if tuple(sorted(self.contexts_ms))!=tuple(self.contexts_ms):raise ValueError('Contexts must grow')
        vals=(self.edge_guard_ms,self.support_ms,self.context_cap_fraction,self.required_margin,self.max_peak_rewrite_fraction,
            self.residual_budget_db,self.energy_change_budget_db,self.envelope_p95_budget_db,self.envelope_max_budget_db,
            self.ms_budget_db,self.envelope_restore_max_gain)
        ...
        if not .05<=self.ms_budget_db<=2 or not 0<=self.envelope_restore_max_gain<=.25:raise ValueError('Invalid stereo/restore gate')
        if not isinstance(self.envelope_restore_iterations,int) or not 0<=self.envelope_restore_iterations<=12:raise ValueError('Invalid restore iterations')
...
def _metrics(r,y,sr,cfg):
    d=y-r;rms=lambda a:float(np.sqrt(np.mean(a*a)))
    resid=_db(rms(d)/max(rms(r),1e-15));energy=_edb(np.mean(y*y)/max(np.mean(r*r),1e-20))
    w=max(3,round(.020*sr));pr=uniform_filter1d(np.mean(r*r,axis=1),w,mode='constant');py=uniform_filter1d(np.mean(y*y,axis=1),w,mode='constant')
    active=pr>max(float(pr.max())*1e-4,1e-20);env=10*np.log10(np.maximum(py[active],1e-20)/np.maximum(pr[active],1e-20)) if np.any(active) else np.zeros(1)
    def ratio(a):
        m=(a[:,0]+a[:,1])*.5;s=(a[:,0]-a[:,1])*.5
        return _db(np.sqrt(np.mean(m*m))/max(np.sqrt(np.mean(s*s)),1e-15))
    exact=np.array_equal(r[:,0],r[:,1]) or np.array_equal(r[:,0],-r[:,1]);ms=0. if exact else ratio(y)-ratio(r)
    gates=dict(residual=resid<=cfg.residual_budget_db,energy=abs(energy)<=cfg.energy_change_budget_db,
        envelope_p95=float(np.percentile(abs(env),95))<=cfg.envelope_p95_budget_db,
        envelope_max=float(np.max(abs(env)))<=cfg.envelope_max_budget_db,ms=abs(ms)<=cfg.ms_budget_db)
    return dict(residual_relative_db=resid,energy_change_db=energy,envelope_20ms_abs_p95_db=float(np.percentile(abs(env),95)),
        envelope_20ms_abs_max_db=float(np.max(abs(env))),ms_ratio_change_db=ms,gates=gates)

def _positive_gain_bound(r,y,ceiling):
    """Largest common beta>=0 such that |y + beta*r| <= ceiling in both channels."""
    n=len(r);bound=np.full(n,np.inf);eps=1e-15
    for ch in range(2):
        x=r[:,ch];z=y[:,ch];b=np.full(n,np.inf);pos=x>eps;neg=x<-eps
        b[pos]=(ceiling-z[pos])/x[pos];b[neg]=(-ceiling-z[neg])/x[neg]
        bound=np.minimum(bound,b)
    return np.maximum(bound,0.)

def _restore_envelope(r,y,sr,ceiling,taper,cfg):
    """Move removed short-time energy into safe neighbours using original signal as carrier."""
    if cfg.envelope_restore_iterations==0 or cfg.envelope_restore_max_gain<=0:return y,0.
    win=max(3,round(.020*sr));smooth=max(3,round(.006*sr));out=y.copy();max_beta=0.
    for _ in range(cfg.envelope_restore_iterations):
        pr=uniform_filter1d(np.mean(r*r,axis=1),win,mode='constant');py=uniform_filter1d(np.mean(out*out,axis=1),win,mode='constant')
        desired=np.sqrt(np.maximum(pr,1e-20)/np.maximum(py,1e-20))-1.;desired=np.clip(desired,0,cfg.envelope_restore_max_gain)
        desired=uniform_filter1d(desired,smooth,mode='nearest')*taper
        bound=_positive_gain_bound(r,out,ceiling);beta=np.minimum(desired,bound*.97)
        beta[~np.isfinite(beta)]=desired[~np.isfinite(beta)];beta=np.clip(beta,0,cfg.envelope_restore_max_gain)
        if float(np.max(beta))<1e-8:break
        out=out+beta[:,None]*r;max_beta=max(max_beta,float(np.max(beta)))
        if float(np.max(np.abs(out)))>ceiling+2e-10:raise kernel.NotFeasible('Envelope redistribution exceeded peak box')
    return out,max_beta
...
def solve_context(r,sr,ceiling,kernel_cfg,*,locked=None,cfg=ContextConfig()):
    ...
    time_w=1+kernel_cfg.quiet_penalty*np.clip(1-local/(local_max*.08),0,1);norm=np.sqrt(np.sum(r*r,axis=1)+1e-18);perp=np.column_stack((-r[:,1],r[:,0]))/norm[:,None]
    support_n=max(3,int(round(cfg.support_ms*sr/1000))|1);need=maximum_filter1d(required,size=support_n,axis=0,mode='constant');amp=maximum_filter1d(np.abs(r),size=support_n,axis=0,mode='constant')
    ...
    low=np.maximum(-ceiling-r,-allowance);high=np.minimum(ceiling-r,allowance);edge=np.zeros_like(r,dtype=bool);edge[:2]=True;edge[-2:]=True
    ...
    def grad(d):
        g=fft.irfft(weights[:,None]*fft.rfft(d,axis=0),n=n,axis=0);g+=time_w[:,None]*d;g+=kernel_cfg.stereo_penalty*np.sum(d*perp,axis=1)[:,None]*perp;return g
    d=np.minimum(np.maximum(np.zeros_like(r),low),high);z=d.copy();t=1.
    for i in range(cfg.iterations):
        nxt=np.minimum(np.maximum(z-grad(z)/L,low),high);nt=.5*(1+math.sqrt(1+4*t*t));ex=nxt+(t-1)/nt*(nxt-d)
