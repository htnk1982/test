...
MODULES = ('processed_finish', 'distribution_finish', 'distribution_peak',
           ...
           'natural_finish','natural_legacy_finish','offline_peak_lab','offline_peak_stream',
           'note_sub_conditioned','natural_gui')
...
def main():
    ...
    manifest = dict(app_version='3.0.0-oppo-exe', resources=resources,
...
from pathlib import Path
a = Analysis([{str(root/'natural_exe_entry.py')!r}], pathex=[{str(root)!r}],
    ...
    noarchive=False)
pyz = PYZ(a.pure)
exe = EXE(pyz, a.scripts, [], exclude_binaries=True, name='PDRM_OPPO',
...
'''
    ...
    with zipfile.ZipFile(sources/'PDRM_APPLICATION_SOURCE.zip', 'w', zipfile.ZIP_DEFLATED) as z:
        for name in MODULES + ('natural_exe_entry','processed_exe_entry'): z.write(root/(name+'.py'), name+'.py')
        ...
        z.write(root/'docs/OPPO_EXE_GUIDE.md', 'OPPO_EXE_GUIDE.md')
...
'''
    ...
    shutil.copy2(root/'docs/OPPO_EXE_GUIDE.md', dest/'使い方.md')
    ...
    if sorted(p.name for p in exes) != ['PDRM_OPPO.exe','ffmpeg.exe']:
