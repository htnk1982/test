"""Build PDRM OPPO v3.1 in a clean Windows environment."""
from pathlib import Path
import argparse,hashlib,importlib.metadata as md,json,os,shutil,subprocess,sys,urllib.request,zipfile

MODULES=('processed_finish','distribution_finish','distribution_peak','accepted_finish','note_sub_lab','note_sub_lab_v02',
'hf_temporal_contrast_lab','target_settings','target_gui','natural_finish','natural_legacy_finish','offline_peak_lab',
'offline_peak_stream','note_sub_conditioned','natural_gui','offline_peak_rescue','offline_peak_stream_v31',
'natural_finish_v31','natural_legacy_finish_v31')
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--codec',required=True,type=Path);args=ap.parse_args()
    codec=args.codec.resolve();root=Path.cwd().resolve()
    ...
    work=root/'build_exe_assets_v31';work.mkdir(exist_ok=True);hooks=work/'hooks';hooks.mkdir(exist_ok=True)
    ...
    manifest=dict(app_version='3.1.0-oppo-exe',resources=resources,source_commit=os.environ.get('GITHUB_SHA','local'),
        versions={n:md.version(n) for n in PACKAGES},ffmpeg_version=version,native_recipe='packaging/build_codec.sh',signing='UNSIGNED')
    ...
    spec=f'''# generated OPPO v3.1\na = Analysis([{str(root/'natural_exe_entry_v31.py')!r}], pathex=[{str(root)!r}],\n binaries=[({str(ff)!r}, 'native')], datas={datas!r},\n hiddenimports={list(MODULES)+['natural_exe_entry','_cffi_backend','imageio_ffmpeg','tkinter','tkinter.filedialog']!r},\n hookspath=[{str(hooks)!r}], excludes=['pytest','IPython','matplotlib','pandas','torch','scipy.tests','numpy.tests'], noarchive=False)\npyz=PYZ(a.pure)\nexe=EXE(pyz,a.scripts,[],exclude_binaries=True,name='PDRM_OPPO',console=True,debug=False,strip=False,upx=False,uac_admin=False)\ncoll=COLLECT(exe,a.binaries,a.datas,strip=False,upx=False,name='PDRM_OPPO')\n'''
    specpath=work/'PDRM_OPPO_v31.spec';specpath.write_text(spec,encoding='utf-8')
    subprocess.run([sys.executable,'-m','PyInstaller','--clean','--noconfirm','--distpath',str(root/'dist'),
        '--workpath',str(root/'build_pyinstaller_v31'),str(specpath)],check=True)
    dest=root/'dist'/'PDRM_OPPO';licenses=dest/'LICENSES';sources=dest/'SOURCES';licenses.mkdir(exist_ok=True);sources.mkdir(exist_ok=True)
    ...
    with zipfile.ZipFile(sources/'PDRM_APPLICATION_SOURCE.zip','w',zipfile.ZIP_DEFLATED) as z:
        for name in MODULES+('natural_exe_entry','natural_exe_entry_v31','processed_exe_entry'):
        ...
        z.write(root/'docs/OPPO_V31_GUIDE.md','OPPO_V31_GUIDE.md')
    (dest/'THIRD_PARTY_NOTICES.md').write_text((root/'dist'/'PDRM_OPPO'/'THIRD_PARTY_NOTICES.md').read_text(encoding='utf-8') if (root/'dist'/'PDRM_OPPO'/'THIRD_PARTY_NOTICES.md').exists() else 'Third-party licenses are retained in LICENSES and SOURCES.\n',encoding='utf-8')
    shutil.copy2(root/'docs/OPPO_V31_GUIDE.md',dest/'使い方.md')
    ...
    if exes!=['PDRM_OPPO.exe','ffmpeg.exe']:raise RuntimeError('Unexpected executable: '+repr(exes))
    print(dest)
