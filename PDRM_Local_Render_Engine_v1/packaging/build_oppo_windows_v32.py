"""Build PDRM OPPO v3.2 in a clean Windows environment."""
from pathlib import Path
import argparse,hashlib,importlib.metadata as md,json,os,shutil,subprocess,sys,urllib.request,zipfile

MODULES=('processed_finish','processed_finish_v32','distribution_finish','distribution_peak','accepted_finish','note_sub_lab','note_sub_lab_v02',
...
'natural_finish_v31','natural_legacy_finish_v31','offline_peak_context','offline_peak_stream_v32',
'natural_finish_v32','natural_legacy_finish_v32','workspace_cleanup')
...
def main():
    ...
    work=root/'build_exe_assets_v32';work.mkdir(exist_ok=True);hooks=work/'hooks';hooks.mkdir(exist_ok=True)
    ...
    manifest=dict(app_version='3.2.0-oppo-exe',resources=resources,source_commit=os.environ.get('GITHUB_SHA','local'),
    ...
    with zipfile.ZipFile(sources/'PDRM_APPLICATION_SOURCE.zip','w',zipfile.ZIP_DEFLATED) as z:
        for name in MODULES+('natural_exe_entry','natural_exe_entry_v31','natural_exe_entry_v32','processed_exe_entry'):
        ...
        z.write(root/'docs/OPPO_V32_GUIDE.md','OPPO_V32_GUIDE.md')
    ...
    shutil.copy2(root/'docs/OPPO_V32_GUIDE.md',dest/'使い方.md')
