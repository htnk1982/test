"""Build PDRM AUTO v3.4 in a clean Windows environment."""
...
MODULES=('processed_finish','processed_finish_v32','distribution_finish','distribution_peak','accepted_finish','note_sub_lab','note_sub_lab_v02','hf_temporal_contrast_lab',
 ...
 'offline_peak_context_v34','offline_peak_stream_v34','auto_peak_v34','auto_conditioned_v34','natural_finish_v34','natural_gui_v34')
...
def main():
    ...
    work=root/'build_exe_assets_v34';work.mkdir(exist_ok=True);hooks=work/'hooks';hooks.mkdir(exist_ok=True)
    ...
    spec=f'''# generated AUTO v3.4\na = Analysis([{str(root/'natural_exe_entry_v34.py')!r}], pathex=[{str(root)!r}],\n binaries=[({str(ff)!r}, 'native')], datas={datas!r},\n hiddenimports={list(MODULES)+['natural_exe_entry','natural_exe_entry_v34','_cffi_backend','imageio_ffmpeg','tkinter','tkinter.filedialog']!r},\n hookspath=[{str(hooks)!r}], excludes=['pytest','IPython','matplotlib','pandas','torch','scipy.tests','numpy.tests'], noarchive=False)\npyz=PYZ(a.pure)\nexe=EXE(pyz,a.scripts,[],exclude_binaries=True,name='PDRM_AUTO',console=True,debug=False,strip=False,upx=False,uac_admin=False)\ncoll=COLLECT(exe,a.binaries,a.datas,strip=False,upx=False,name='PDRM_AUTO')\n'''
    specpath=work/'PDRM_AUTO_v34.spec';specpath.write_text(spec,encoding='utf-8')
    subprocess.run([sys.executable,'-m','PyInstaller','--clean','--noconfirm','--distpath',str(root/'dist'),'--workpath',str(root/'build_pyinstaller_v34'),str(specpath)],check=True)
    ...
    shutil.copytree(codec/'LICENSES',licenses/'native-codec',dirs_exist_ok=True);shutil.copytree(codec/'SOURCES',sources/'native-codec',dirs_exist_ok=True);(licenses/'FFMPEG_VERSION_AND_CONFIG.txt').write_text(version,encoding='utf-8')
    ...
    with zipfile.ZipFile(sources/'PDRM_APPLICATION_SOURCE.zip','w',zipfile.ZIP_DEFLATED) as z:
        for name in MODULES+('natural_exe_entry','natural_exe_entry_v34','processed_exe_entry'):z.write(root/(name+'.py'),name+'.py')
    ...
    manifest.update(python=sys.version,libsndfile=libver,pyinstaller=md.version('pyinstaller'));(dest/'BUILD_REPORT.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8')
