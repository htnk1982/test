...
def main():
    ...
    with TemporaryDirectory(prefix='pdrm_p01_build_') as td:
        ...
        readme=f'''PDRM P01 Private Calibration Bundle\n===================================\nEngineering calibration only. Product release: NO.\nAccepted P03-A code baseline: 50a4592e45b3f810e905041c25cff1c3e35b2a88\nBundle build commit: {commit}\n\n使い方:\n1. RUN_P01_CALIBRATION.cmd をダブルクリック。\n2. 最初は「11 - Traces_demo_44k (delimit).wav」を選択。\n3. canonical reference.zip（24曲MP3）を選択。\n4. 元音源フォルダとは別の結果保存フォルダを選択。\n5. 完了後、PDRM_P01_<曲名>_<hash>/ の MASTER.wav と LISTEN_320kbps.mp3 を聴く。\n6. CALIBRATION_MANIFEST.jsonをChatGPTへ戻す。聴感は「改善 / 過剰 / 不足 / 違和感あり（時刻）」だけでよい。\n\n失敗時:\n- 選択した保存先に P01_FAILURE.json を自動保存する。\n- 音源やスクリーンショットではなく、そのJSONだけをChatGPTへ戻す。\n\nプライバシー/契約:\n- 音源はこのPC上だけで処理される。\n- GitHub/CI/ネットワークへ私有音源を送信する機能はない。\n- reference.zipのMP3はローカル一時領域でPCM float WAVへデコードし、既存lossless契約へ渡す。\n- 一時デコードWAVは処理終了時に削除し、global integration contractは緩和しない。\n- Spleeter stem音声は保存せず、完成音にも混ぜない。\n- 元音源は上書きしない。\n\n注意:\n- 初回は24曲reference群の校正を行うため時間がかかる。\n- これはP01/P03実曲校正用であり製品版ではない。\n'''
        ...
        env['PYTHONUTF8']='1';execute([str(isolated/'PDRM_P01_Calibrate.exe'),'--self-test','--self-test-output',str(testout)],EVIDENCE/'SELFTEST.log',build,env,timeout=1800)
        summary=json.loads((testout/'P01_BUNDLE_SELFTEST.json').read_text(encoding='utf-8'))
        if not summary.get('success') or summary.get('observer_provider')!='spleeter_runtime48' or summary.get('reference_count')!=24 or summary.get('reference_codec')!='MP3' or summary.get('global_lossless_contract_relaxed') is not False:
            raise RuntimeError('Frozen private-calibration self-test failed')
