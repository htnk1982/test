...
OUT=ROOT/'P08_PRODUCT_BUNDLE'
EVIDENCE=ROOT/'P08_PRODUCT_BUNDLE_EVIDENCE'
...
def execute(command,log,cwd,env=None,timeout=2400):
    with Path(log).open('w',encoding='utf-8') as f:
        r=subprocess.run([str(v) for v in command],cwd=cwd,env=env,stdout=f,stderr=subprocess.STDOUT,timeout=timeout)
    if r.returncode:
        text=Path(log).read_text(encoding='utf-8',errors='replace');print(text[-25000:],flush=True)
        raise RuntimeError('P08 product bundle step failed: '+str(log))
...
def main():
    ...
    with TemporaryDirectory(prefix='pdrm_p08_product_build_') as td:
        ...
        hidden=[
            'product_app_v52','product_worker_v52','product_gui_runtime_v52','product_processed_v52','product_selftest_v52',
            'natural_gui_v34','natural_gui_v44','gui_runtime_v44','target_settings',
            'p01_private_calibration_bootstrap','p01_private_calibration_entry',
            ...
            'spleeter_observer_adapter_v48','observer_runtime_client_v47',
            'processed_integration','processed_finish','crash_recovery_v43',
        ...
        readme=f'''PDRM Windows Product Candidate v52\n=================================\nPlanner: {PLANNER}\nFinalizer: {FINALIZER}\nDefault WAV: -12 LUFS-I / -2 dBTP\nDefault MP3: -14 LUFS-I / -2 dBTP\nBuild commit: {commit}\n\nDouble-click PDRM.exe. Select one or more original WAV/FLAC files, then the canonical reference.zip.\nThe reference is decoded only to temporary local lossless files. Original audio is never overwritten.\nSpleeter stems are analysis evidence only and are never exported or mixed into the master.\n\nThis build remains a P08 candidate until artifact roundtrip, recovery regression and one final private listening acceptance are complete.\n'''
        (dist/'README_PDRM.txt').write_text(readme,encoding='utf-8')

        isolated=build/'日本語 空白'/'PDRM 製品候補';isolated.parent.mkdir();shutil.copytree(dist,isolated)
        testout=EVIDENCE/'SELFTEST';testout.mkdir();env=dict(os.environ)
        ...
        required=(summary.get('success') and summary.get('frozen') and summary.get('planner_id')==PLANNER and
                  summary.get('observer_provider')=='spleeter_runtime48' and summary.get('source_unchanged') and
                  summary.get('stem_audio_in_master') is False)
        if not required:raise RuntimeError('Frozen P08 product self-test failed: '+json.dumps(summary))

        final=OUT/'PDRM_Windows';shutil.copytree(dist,final)
        report=dict(
            success=True,product_release=False,stage='P08_B_CANDIDATE',commit=commit,planner=PLANNER,finalizer=FINALIZER,
            platform=platform.platform(),versions=versions,exe_sha256=sha(final/'PDRM.exe'),
            exe_bytes=(final/'PDRM.exe').stat().st_size,
            bundle_bytes=sum(p.stat().st_size for p in final.rglob('*') if p.is_file()),
            runtime_bytes=sum(p.stat().st_size for p in (final/'PDRM_OBSERVER_RUNTIME').rglob('*') if p.is_file()),
            runtime_manifest_sha256=summary.get('observer_runtime_manifest_sha256'),
            frozen_selftest=summary,private_audio_used=False,private_audio_upload_path=False,user_python_required=False,
        )
        (EVIDENCE/'SUMMARY.json').write_text(json.dumps(report,ensure_ascii=False,indent=2,allow_nan=False),encoding='utf-8')
        print('P08_PRODUCT_BUNDLE '+json.dumps(report,ensure_ascii=True),flush=True)
