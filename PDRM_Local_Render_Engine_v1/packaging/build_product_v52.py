...
def main():
    ...
    with TemporaryDirectory(prefix='pdrm_p08_product_build_') as td:
        ...
        required=(summary.get('success') and summary.get('frozen') and summary.get('planner_id')==PLANNER and summary.get('observer_provider')=='spleeter_runtime48' and summary.get('source_unchanged') and summary.get('stem_audio_in_master') is False and summary.get('precomputed_calibration_valid') and summary.get('reference_free_product_request') and summary.get('status_read_retry') is True and summary.get('status_runtime_version')=='gui-runtime-v0.2.2')
        if not required:raise RuntimeError('Frozen P08 product/precomputed-calibration/status-channel self-test failed: '+json.dumps(summary))
        final=OUT/'PDRM_Windows';shutil.copytree(dist,final)
        report=dict(success=True,product_release=False,stage='P08_B_CANDIDATE_V56_STATUS_CHANNEL_HARDENED',commit=commit,planner=PLANNER,finalizer=FINALIZER,platform=platform.platform(),versions=versions,exe_sha256=sha(final/'PDRM.exe'),exe_bytes=(final/'PDRM.exe').stat().st_size,bundle_bytes=sum(p.stat().st_size for p in final.rglob('*') if p.is_file()),runtime_bytes=sum(p.stat().st_size for p in (final/'PDRM_OBSERVER_RUNTIME').rglob('*') if p.is_file()),runtime_manifest_sha256=summary.get('observer_runtime_manifest_sha256'),precomputed_calibration_sha256=summary.get('calibration_sha256'),frozen_selftest=summary,private_audio_used=False,private_audio_upload_path=False,user_python_required=False,reference_audio_bundled=False,reference_audio_cached=False,precomputed_calibration_metadata=True,status_telemetry_non_authoritative=True)
