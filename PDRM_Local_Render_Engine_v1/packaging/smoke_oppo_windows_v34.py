...
def main():
    original=ROOT/'dist'/'PDRM_AUTO';area=Path(tempfile.mkdtemp(prefix='AUTO34_EXE_',dir=os.environ.get('RUNNER_TEMP')))
    ...
    checks=[];measurements=[]
    def run(args,expected=0,timeout=900):
        mode=[] if args and str(args[0]) in ('--bundle-check','--gui-check') else ['--headless']
        p=subprocess.run([str(exe),'--no-pause','--no-open',*mode,*map(str,args)],cwd=area,env=env,capture_output=True,timeout=timeout)
    ...
    assert b['frozen'] and Path(b['ffmpeg']).samefile(ff) and g['status']=='PASS' and g['automatic_policy']=='GAIN_ONLY_OPPO_LIMITER' and g['execution']=='SPARSE_EXACT_CONTEXT'
    for name in ('offline_peak_context_v34.py','offline_peak_stream_v34.py','auto_peak_v34.py','auto_conditioned_v34.py','natural_finish_v34.py','natural_gui_v34.py'):assert name in b['manifest']['resources']
    checks.append('Bundle integrity, sparse-exact v3.4 modules, bundled FFmpeg and target-only GUI')
    ...
    run(args)
    work=local/'PDRM_Local_Render_Engine_v1'/'oppo_finish_v34';assert not list(work.rglob('*.wav')) and not list(work.rglob('*.mp3')) and tree_bytes(work)<5*1024*1024
