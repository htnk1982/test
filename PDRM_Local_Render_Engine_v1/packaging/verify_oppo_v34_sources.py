"""Verify v3.4.1 against unit specifications, not buggy context behavior.

Shared stream_v32 is deliberately not frozen: its ms conversion was defective.
The accepted kernel, weight/gate algorithms and other frozen DSP remain checked.
"""
...
def main():
    ...
    frozen=['offline_peak_lab.py','offline_peak_stream.py','offline_peak_rescue.py','offline_peak_stream_v31.py','offline_peak_context.py',
    ...
    for pattern in ('test_note_sub*.py','test_hf_temporal_contrast.py','test_accepted_finish.py','test_distribution_finish.py','test_processed_finish.py','test_target_controls.py',
        'test_offline_peak_lab.py','test_offline_peak_stream.py','test_offline_peak_rescue.py','test_offline_peak_context.py','test_auto_policy_v33.py','test_oppo_performance_v34.py','test_context_units.py'):
    ...
    report=dict(tests=r.testsRun,ok=r.wasSuccessful(),failures=len(r.failures),errors=len(r.errors),skipped=len(r.skipped),python=sys.version,platform=sys.platform,
        kernel_sha256=selected.verify_kernel(),v34='3.4.1: corrected millisecond bounds + fixed short-frame DSP; buggy rescue is not the reference',
    ...
    sys.exit(0 if r.wasSuccessful() and r.testsRun>=272 and not r.skipped else 1)
