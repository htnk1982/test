    ...
    with tempfile.TemporaryDirectory(prefix='pdrm_p07b_') as tmp:
        ...
        with worker_log.open('w',encoding='utf-8') as log:
            ...
            if owner is None:
                ...
                proc.wait(timeout=10);log.flush()
                tail=worker_log.read_text(encoding='utf-8',errors='replace')[-4000:]
            ...
            assert marker['source_sha256']==original.file_sha256
        ...
        assert stale_bytes>=8*1024*1024
...
def main():
    ...
    try:summary['acceptance']=acceptance()
        ...
        summary.update(success=False,acceptance_error=repr(exc));dump(OUT/'SUMMARY.json',summary);raise
    summary.update(product_release=False,private_music=False,physical_power_loss_tested=False,
        process_kill_tested=True,capacity_failure_tested=True,parent_P07_done=False)
    dump(OUT/'SUMMARY.json',summary)
    md=f'''# PDRM P07-B — 強制終了・容量障害の回収 実行記録\n\n日付: 2026-09-10。Issue #5。commit `{summary['commit']}`。環境: {summary['platform']}。\n\n実プロセスでPDRMと同じsealed owned-workspaceを作り、未完了の大きな作業物をfsyncした後にOSのkill APIで終了させた。生存中は回収しない、kill後は残る、次の実際のnew-chain処理が死亡ownerだけを回収してWAV/MP3まで完走する、を一つの受入で確認した。\n\n所有判定はsource hash、request hash、PID、process create time、seal。PIDだけでは判定しない。別source、live owner、seal不成立folderは削除しない。これは悪意あるローカル攻撃に対するsandboxではなく、誤削除を避ける運用境界。\n\n容量不足はwrapper経由でfree=1byteを注入し、DSP開始関数を呼ぶと失敗する仕掛けで、DSP前にOSErrorとなり公開WAV/MP3が作られないことを確認。物理ディスクを実際に満杯にした試験ではない。\n\n通常のコピー/I/O例外、変更済みprocessed、backup保護、通常中断のjournal回復はP07-Aの既存試験を同時再実行。ソフトウェア試験{summary['tests']}件。これを製品完成率や音質受入に換算しない。物理的な電源断、SSD故障、長時間多曲の容量累積はP07-C/P08へ残る。\n'''
