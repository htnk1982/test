...
LAB opt-in is mandatory. Publication COMPLETE is not musical approval. All
input/snapshot and completed-output proofs remain checked. Across a fresh
rerender, a temporary-container snapshot is not the delivered audio identity.
Normal cleanup owns one call directory; hard-death recovery is separate P07-B.
"""
...
VERSION='processed-integration-lab-1.0.1'
SCHEMA=1
io=publisher.io
...
def _read_sidecar(path,request):
    ...
    if value.get('publication_scope')!='LAB_TECHNICAL_ONLY' or not valid_hash(value.get('lowend_pcm_sha256')):
        raise RuntimeError('Invalid LAB output/PCM identity')
    return value


def same_delivery(previous,current):
    """Fresh verified snapshots may have different temporary file containers.

    Never relax source/request/PCM/output-file/route/metrics equivalence. The
    first verified snapshot remains recorded; it is not relabelled as the latest
    rendering. Each new rendering has separately passed finish._proof and its
    original/physical snapshot checks before reaching this comparison.
    """
    excluded={'snapshot_sha256','sha256'}
    a={k:v for k,v in previous.items() if k not in excluded}
    b={k:v for k,v in current.items() if k not in excluded}
    return a==b
...
class _Backend:
    ...
    def __init__(self,planner,render_backend,progress=None):
        ...
        self.planner=planner;self.planner_identity=_plain(planner.identity())
        self.render_backend=render_backend;self.progress=progress
    ...
    def run_file(self,source,root,*,targets=None,write_mp3=True):
        ...
        verified=finish._proof(final,raw['identity'])
        comparable=dict(raw);comparable.pop('rerun_status',None)
        if comparable!=verified:raise RuntimeError('Returned report differs from sealed completion report')
        if (raw.get('status')!='LAB_RENDER_COMPLETE_NOT_LISTENING_APPROVED' or raw.get('source_unchanged') is not True or
            raw.get('old_note_sub_called') is not False or raw.get('render_backend')!=self.render_backend or
            raw.get('lowend_assessment') not in ASSESSMENTS or raw['identity']['planner']!=self.planner_identity or raw['identity']['write_mp3'] is not True):
        ...
        evidence=dict(schema=SCHEMA,version=VERSION,request=self.request,
            ...
            lowend_pcm_sha256=raw['lowend_report']['output_identity']['pcm_sha256'],
            ...
            work_cleanup='OWNED_CALL_DIRECTORY_ON_NORMAL_EXIT',hard_termination_recovery='NOT_YET_INTEGRATED')
        ...
        if self.sidecar.exists():
            ...
            if not same_delivery(previous,evidence):
                keys=sorted(k for k in set(previous)|set(evidence) if k not in ('sha256','snapshot_sha256') and previous.get(k)!=evidence.get(k))
                raise RuntimeError('Rerender delivery differs; nothing overwritten; fields='+','.join(keys))
            self.replay_snapshot_changed=previous['snapshot_sha256']!=evidence['snapshot_sha256']
            print('PUBLICATION_REPLAY '+json.dumps(dict(exact_output_files_match=True,exact_lowend_pcm_match=True,
                temporary_snapshot_changed=self.replay_snapshot_changed,source_request_and_metrics_match=True)),flush=True)
            # Preserve the first verified proof for these same delivered bytes.
        else:
    ...
    def verify_final(self,final,identity):
        ...
        if raw['lowend_assessment']!=receipt['lowend_assessment'] or raw['lowend_report']['output_identity']['pcm_sha256']!=receipt['lowend_pcm_sha256']:
            raise RuntimeError('Musical assessment/lowend PCM changed')
...
def run_file(source,work_root=None,*,planner=None,targets=None,render_backend='joint-v42',
    ...
    """Journaled sibling processed pair; not a filesystem-wide atomic pair.

    New-chain publication only, with explicit LAB opt-in. Normal interruption
    rerenders from original, validates exact delivered hashes, then resumes the
    existing journal. GUI/default product routing and hard-crash recovery remain
    separate tasks. No complete render cache is kept per normally ended song.
    """
    ...
    backend.sidecar=meta/(key+'.'+digest(backend.request)+'.integration.json')
    ...
    with io.job_lock(lock):
        ...
        return dict(result,publication_scope='LAB_TECHNICAL_ONLY',quality_status=evidence['quality_status'],
            lowend_assessment=evidence['lowend_assessment'],processing_evidence=str(backend.sidecar),final_cache_removed=True,
            replay_verified_snapshot_changed=backend.replay_snapshot_changed,hard_termination_recovery='NOT_YET_INTEGRATED'),dest
