...
class Config:
    ...
    def validate(self):
        ...
        if not 0 < self.edge_guard_seconds <= .20:
            raise ValueError('Invalid edge guard')
        if not 0 < self.max_source_fundamental_ratio <= .25 or not 0 < self.max_source_focus_upper_fraction <= .10:
            raise ValueError('Invalid source-shape limits')
        if not .25 <= self.max_drum_share < .75 or not 0 <= self.max_vocal_share <= .25 or not 0 <= self.max_vocal_context_disagreement <= .25:
            raise ValueError('Invalid role limits')
        if not .5 <= self.min_supported_fraction <= 1 or not .5 <= self.min_periodicity <= 1:
...
def _source_vetoes(source_proof, source_shape, cfg):
    ...
    reasons = []
    if not source_proof or source_proof.get('method') != 'ORIGINAL_COMPONENT_PRESENT;_HARMONIC_SUPPORTED;_NO_MISSING_FUNDAMENTAL':
        reasons.append('ABSTAIN_NO_ORIGINAL_F0_PROOF')
    if ratio > cfg.max_source_fundamental_ratio:
        reasons.append('ABSTAIN_F0_NOT_WEAK_ENOUGH_FOR_REPAIR')
    if upper_fraction > cfg.max_source_focus_upper_fraction:
        reasons.append('ABSTAIN_SOURCE_TOO_HARMONICALLY_BRIGHT_FOR_SUB_REPAIR')
    return reasons, ratio, upper_fraction


def permit(event, arrays, meta, source_digest, source_proof, source_shape, cfg=Config()):
    cfg.validate()
    baseline = legacy.permit(event, arrays, meta, source_digest)
    ...
    if not baseline['allowed']:
        ...
        if structural:
            return dict(
                ...
                permission_path='LEGACY_DENIAL_PRESERVED',
                envelope_role_indices=[2],
    ...
    if baseline['allowed']:
        if source_reasons:
            return dict(
                ...
                permission_version=VERSION,
                permission_path='STRICT_ROLE_SOURCE_VETO',
                envelope_role_indices=[2],
                source_shape=source_shape,
                source_proof=source_proof,
                source_veto_reasons=source_reasons,
        ...
        return dict(
            baseline,
            permission_version=VERSION,
            permission_path='LEGACY_STRICT_WITH_SOURCE_VETO',
            envelope_role_indices=[2],
            source_shape=source_shape,
            source_proof=source_proof,
            source_veto_reasons=[],
    ...
    t = legacy._check(arrays, meta, source_digest)
    a = float(event['start'])
    b = float(event['end'])
    f0 = float(event['source_f0_hz'])
    target = float(event['target_hz'])
    if abs(1200 * math.log2(f0 / target)) > cfg.pitch_tolerance_cents:
        reasons.append('DENY_NEW_OCTAVE')
    keep = (t >= a + cfg.edge_guard_seconds) & (t < b - cfg.edge_guard_seconds)
    if keep.sum() < 6:
        reasons.append('ABSTAIN_INSUFFICIENT_INTERIOR_FRAMES')
    ...
    other = roles[:, :, 2]
    vocals = roles[:, :, 3]
    vocal_disagreement = np.abs(vocals[0] - vocals[1])

    pitch = np.asarray(arrays['bass_f0_hz'], float)
    ...
    role_reliable = (
        ...
        & (vocal_disagreement <= cfg.max_vocal_context_disagreement)
    )
    pitch_reliable = (
        (cents.max(axis=0) <= cfg.pitch_tolerance_cents)
    ...
    out = dict(
        baseline,
        allowed=not reasons,
        reason_codes=reasons or ['ALLOW_EXISTING_WEAK_F0_FALLBACK'],
        permission_version=VERSION,
        permission_path='EXISTING_WEAK_F0_FALLBACK',
        source_proof=source_proof,
        source_shape=source_shape,
        source_veto_reasons=source_reasons,
        ...
        fallback_vocal_context_disagreement_p95=float(np.percentile(vocal_disagreement[keep], 95)) if np.any(keep) else 1.,
    )
    if out['allowed']:
