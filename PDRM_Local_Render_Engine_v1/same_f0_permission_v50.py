...
This is NOT a general bass-role relaxation. It is used only when the accepted
v37 event gate abstains on local role/pitch, while the original 2mix proves that
the requested f0 already exists and is weak. A source spectral-shape veto and
strict drum/vocal veto protect against deep voice and kick false positives.
"""
...
VERSION='same-f0-permission-lab-0.1.0'
...
def permit(event,arrays,meta,source_digest,source_proof,source_shape,cfg=Config()):
    ...
    if baseline['allowed']:
        return dict(baseline,permission_version=VERSION,permission_path='LEGACY_STRICT',envelope_role_indices=[2],source_shape=source_shape,source_proof=source_proof)
    # Never override structural denials such as octave creation, target range,
    # duration, missing observer, clock mismatch or outside-observation geometry.
    if baseline.get('reason_codes')!=['ABSTAIN_LOCAL_ROLE_OR_PITCH']:
        return dict(baseline,permission_version=VERSION,permission_path='LEGACY_DENIAL_PRESERVED',envelope_role_indices=[2],source_shape=source_shape,source_proof=source_proof)
    ...
    if ratio>cfg.max_source_fundamental_ratio:reasons.append('ABSTAIN_F0_NOT_WEAK_ENOUGH_FOR_FALLBACK')
    if upper_fraction>cfg.max_source_focus_upper_fraction:reasons.append('ABSTAIN_SOURCE_TOO_HARMONICALLY_BRIGHT_FOR_SUB_FALLBACK')
    ...
    p=np.asarray(arrays['low_power'],float)+np.asarray(arrays['body_power'],float)
    roles=p[:,:,1:]/np.maximum(p[:,:,1:].sum(axis=2,keepdims=True),1e-24)
    drums=roles[:,:,0];instrument=roles[:,:,1]+roles[:,:,2];vocals=roles[:,:,3]
    grouped=np.stack((drums,instrument,vocals),axis=2);disagreement=np.sum(abs(grouped[0]-grouped[1]),axis=1)/2
    ...
    # Bass pitch is still required. 'other' can rescue role assignment but cannot
    # invent pitch evidence. Kick has zero bass pitch/periodicity and is denied.
    ...
    out=dict(baseline,allowed=not reasons,reason_codes=reasons or ['ALLOW_EXISTING_WEAK_F0_FALLBACK'],permission_version=VERSION,permission_path='EXISTING_WEAK_F0_FALLBACK',
        source_proof=source_proof,source_shape=source_shape,envelope_role_indices=[2,3],fallback_supported_fraction=fraction,
        fallback_instrument_share_q20=float(np.percentile(instrument.min(axis=0)[keep],20)) if np.any(keep) else 0.,
        ...
        fallback_grouped_disagreement_p95=float(np.percentile(disagreement[keep],95)) if np.any(keep) else 1.)
    if out['allowed']:
